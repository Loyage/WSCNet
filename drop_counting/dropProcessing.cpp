...
Parameter::Parameter(int kernelSize, float minRadius, float maxRadius, float areaRate, bool findOverlap, bool parameterAdjust,bool visualization,int waitTime) :
kernel_size(kernelSize), min_radius(minRadius), max_radius(maxRadius), area_rate(areaRate),findOverLap(findOverlap), parameter_adjust(parameterAdjust), isVisualization(visualization), wait_time(waitTime)
{
	;
}
...
void findSingleDrop(const Mat &src_bw_img, ScoreCircle &scoreCircles, const Parameter &param, int dev)
{
	...
	float areaRate = param.area_rate;
	...
	for (int i = 0; i < pCountour_all.size(); i++)
	{
		...
		if (area_countour / area_circle > areaRate &&area_whole_circle < max_area)//计算轮廓区域面积与圆mask面积之比
		{
